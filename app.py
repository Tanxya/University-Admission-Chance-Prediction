...
def main():
    ...
    page = st.selectbox(
        "Know more about:",
        ["This App"] + [f"Parameters - {p}" for p in ["GRE", "TOEFL", "University Rating", "SOP", "LOR", "CGPA", "Research"]] + ["Predictor"]
    )
    load_page(page)
    ...
    if st.button("Let's Go"):
        ...
        if sys.platform == "win32":  # For Windows
            subprocess.Popen(['start', 'cmd', '/k', 'streamlit run D:\\Sem-6\\LABS\\MiniProject\\project\\pages\\predictor.py'], shell=True)
        elif sys.platform == "darwin":  # For macOS
            subprocess.Popen(['open', '-a', 'Terminal', 'streamlit run D:\\Sem-6\\LABS\\MiniProject\\project\\pages\\predictor.py'])
        else:  # For Linux
            subprocess.Popen(['gnome-terminal', '--', 'streamlit run D:\\Sem-6\\LABS\\MiniProject\\project\\pages\\predictor.py'])
